...
from yuyao.data.data_loader import Cloth1M_loader
...
parser.add_argument('--dataset', type=str, default='cloth1m',
                    help='dataset')
...
parser.add_argument('--est_epochs', default=100, type=int,
                    help='number of total estimation epochs to run')
...
def run_forward(train_loader, val_loader, test_loader, args, t_m = np.eye(10), out_dir="./out_dir", epochs=100):
    print(args.arch)
    model = models.__dict__[args.arch](num_classes=args.num_classes,input_channel=args.input_channel, pretrained = args.pretrained)
    model = model.cuda()
    best_acc1 = 0
...
def main():
    ...
    _, train_loader, val_loader, est_loader, test_loader = Cloth1M_loader(
        num_workers = 0, 
        batch_size = 128, 
    ...
    T_mentornet_acc = run_mentoring(train_dataset, test_dataset, T_noise_rate, input_channel=args.input_channel, arch=args.arch, pretrained=args.pretrained, num_classes = args.num_classes, n_epoch = args.cl_epochs)
    print("T-MentorNet acc", T_mentornet_acc)

    print("++++++++++++run DT-MentorNet+++++++++++++++++")
    DT_mentornet_acc = run_mentoring(train_dataset, test_dataset, DT_noise_rate, input_channel=args.input_channel, arch=args.arch, pretrained=args.pretrained, num_classes = args.num_classes, n_epoch = args.cl_epochs)
    print("DT-MentorNet acc", DT_mentornet_acc)

    print("++++++++++++run Decoupling+++++++++++++++++") 
    decoupling_acc = run_decoupling(train_dataset, test_dataset, T_noise_rate, input_channel = args.input_channel, arch=args.arch, pretrained=args.pretrained, num_classes = args.num_classes, n_epoch = args.cl_epochs)
    print("Decoupling acc", decoupling_acc)

    print("++++++++++++run Mixup+++++++++++++++++") 
    mixup_acc = run_mixup(train_dataset, test_dataset, num_classes = args.num_classes, input_channel=args.input_channel,  arch=args.arch, pretrained=args.pretrained, n_epoch = args.cl_epochs)
    print("Mixup acc", mixup_acc)

    print("++++++++++++run T-Coteaching+++++++++++++++++") 
    T_coteaching_acc = run_coteaching(train_dataset, test_dataset, T_noise_rate, input_channel = args.input_channel, arch=args.arch, pretrained=args.pretrained, num_classes = args.num_classes, n_epoch = args.cl_epochs)
    print("T-Coteaching acc", T_coteaching_acc)   

    print("++++++++++++run DT-Coteaching+++++++++++++++++")
    DT_coteaching_acc = run_coteaching(train_dataset,test_dataset, DT_noise_rate, input_channel = args.input_channel, arch=args.arch, pretrained=args.pretrained, num_classes = args.num_classes, n_epoch = args.cl_epochs)
